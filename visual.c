...
typedef struct _ParamQueueEntry ParamQueueEntry;
...
struct _GstVisual
{
  ...
  /* Param get and set queues */
  VisList set_property_queue;
  int set_property_flag;
...
static void libvisual_log_handler (const char *message, const char *funcname,
    void *priv);
static void gst_visual_install_params(GstVisualClass *klass);
static void gst_visual_set_property  (GObject      *object,
                         guint         prop_id,
                         GValue *value,
                         GParamSpec   *pspec);
static void _gst_visual_set_property  (GObject      *object,
                         guint         prop_id,
                         const GValue *value,
                         GParamSpec   *pspec);
static void _gst_visual_get_property  (GObject      *object,
                         guint         prop_id,
                         GValue       *value,
                         GParamSpec   *pspec);
static void gst_visual_handle_property_changes(GstVisual *visual);
static void make_valid_name (char *name);
...
gst_visual_class_init (gpointer g_class, gpointer class_data)
{
  ...
  gst_visual_install_params(klass);

  object->dispose = gst_visual_dispose;
}

static void
gst_visual_install_params(GstVisualClass *class) 
{
    ...
    object_class->set_property = _gst_visual_set_property;
    object_class->get_property = _gst_visual_get_property;
    ...
    if(class->plugin == NULL)
        return;

    visplugin = visual_plugin_load (class->plugin);

    visual_log_return_if_fail(visplugin != NULL);
    
    visual_plugin_realize(visplugin);

    params = visual_plugin_get_params(visplugin);
    visual_log_return_if_fail(params != NULL);

    plist = &params->entries;
    visual_log_return_if_fail(list != NULL);

    param_count = plist->count;

    for(j = 0; j < param_count; j++) {
        param = visual_list_get(plist, j);
        if(param == NULL) {
            continue;
        }

        name = visual_param_entry_get_name(param);
        make_valid_name(name);

        switch(param->type) {
        case VISUAL_PARAM_ENTRY_TYPE_STRING:
            g_object_class_install_property(object_class, j + 1,
                g_param_spec_string(name, name, visual_param_entry_get_annotation(param), 
                    "", G_PARAM_READWRITE));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_INTEGER:
            g_object_class_install_property(object_class, j + 1,
                g_param_spec_int(name, name, visual_param_entry_get_annotation(param), 
                    visual_param_entry_min_get_integer(param), 
                    visual_param_entry_max_get_integer(param), 
                    visual_param_entry_default_get_integer(param), G_PARAM_READWRITE));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_FLOAT:
            g_object_class_install_property(object_class, j + 1,
                g_param_spec_float(name, name, visual_param_entry_get_annotation(param), 
                    visual_param_entry_min_get_float(param), 
                    visual_param_entry_max_get_float(param),
                    visual_param_entry_default_get_float(param), G_PARAM_READWRITE));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_DOUBLE:
            g_object_class_install_property(object_class, j + 1,
                g_param_spec_double(name, name, visual_param_entry_get_annotation(param), 
                    visual_param_entry_min_get_double(param), 
                    visual_param_entry_max_get_double(param),
                    visual_param_entry_default_get_double(param), G_PARAM_READWRITE));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_COLOR:
            g_object_class_install_property(object_class, j + 1,
                g_param_spec_uint(name, name, "Please read actor documentation",
                    0, UINT_MAX, 0x0, G_PARAM_READWRITE));
            break;
        default:
            break;
        }
...
static void _gst_visual_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec)
{
    GstVisual *visual = GST_VISUAL(object);
    ParamQueueEntry *entry = visual_mem_malloc0(sizeof(ParamQueueEntry));

    entry->object = object;
    entry->prop_id = prop_id;
    g_value_init(&entry->set_value, G_VALUE_TYPE(value));
    g_value_copy(value, &entry->set_value);
    entry->pspec = pspec;

    visual_list_add(&visual->set_property_queue, entry);
    visual->set_property_flag = TRUE;

    gst_visual_handle_property_changes(visual);
}

static void gst_visual_set_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) 
{
    GstVisual *visual = GST_VISUAL(object);
    VisParamContainer *params;
    VisPluginInfo *info;
    ...
    visual_log_return_if_fail(visual->actor);

    plugin = visual_actor_get_plugin(visual->actor);
    visual_log_return_if_fail(plugin != NULL);

    info = visual_plugin_get_info(plugin);

    params = visual_plugin_get_params(plugin);
    visual_log_return_if_fail(params != NULL);
    ...
    param = (VisParamEntry *)visual_list_get(list, prop_id - 1);

    switch(visual_param_entry_get_type(param)) {
        case VISUAL_PARAM_ENTRY_TYPE_STRING:
            visual_param_entry_set_string(param, (char *)g_value_get_string(value));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_INTEGER:
            visual_param_entry_set_integer(param, g_value_get_int(value));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_FLOAT:
            visual_param_entry_set_float(param, g_value_get_float(value));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_DOUBLE:
            visual_param_entry_set_double(param, g_value_get_double(value));
            break;
        case VISUAL_PARAM_ENTRY_TYPE_COLOR:
            visual_color_from_uint32(&color, g_value_get_int(value));
...
static void _gst_visual_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec) 
{
    GstVisual *visual = GST_VISUAL(object);
    VisParamContainer *params;
    VisPluginInfo *info;
    ...
    switch(visual_param_entry_get_type(param)) {
        case VISUAL_PARAM_ENTRY_TYPE_STRING:
            s = visual_param_entry_get_string(param);
            g_value_set_string(value, s);
            break;
        case VISUAL_PARAM_ENTRY_TYPE_INTEGER:
            i = visual_param_entry_get_integer(param);
            g_value_set_int(value, i);
            break;
        case VISUAL_PARAM_ENTRY_TYPE_FLOAT:
            f = visual_param_entry_get_float(param);
            g_value_set_float(value, f);
            break;
        case VISUAL_PARAM_ENTRY_TYPE_DOUBLE:
            d = visual_param_entry_get_double(param);
            g_value_set_double(value, d);
            break;
        case VISUAL_PARAM_ENTRY_TYPE_COLOR:
            c = visual_param_entry_get_color(param);
            g_value_set_uint(value, visual_color_to_uint32(c));
            break;
        default:
            break;
    }
...
void gst_visual_handle_property_changes(GstVisual *visual)
{
  /* Check if property changes are queued */
  if(visual->set_property_flag && visual->actor_realized) {
    int count = visual_collection_size(VISUAL_COLLECTION(&visual->set_property_queue));
    int i;
    for(i = count - 1; count >= 0; i--) {
      VisListEntry *entry = visual->set_property_queue.head;
      ParamQueueEntry *qentry;
      int j;
      for(j = 0; j < i && entry; j++)
        entry = entry->next;
      if(entry == NULL)
        break;
      qentry = entry->data;
      gst_visual_set_property(qentry->object, qentry->prop_id, &qentry->set_value, qentry->pspec);
      visual_list_destroy(&visual->set_property_queue, &entry);
    }
    visual->set_property_flag = FALSE;
  }

}

static int gst_visual_queue_destroyer(void *data)
{
...
gst_visual_init (GstVisual * visual)
{
  ...
  visual_collection_set_destroyer(VISUAL_COLLECTION(&visual->set_property_queue), 
    gst_visual_queue_destroyer);
}
...
gst_visual_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      visual->actor_realized = TRUE;

      visual_plugin_events_pump(visual->actor->plugin);
...
{
  guint i, count, transcount;
  VisList *list, *translist;
  ...
  list = visual_actor_get_list ();
  translist = visual_transform_get_list();

#if !defined(VISUAL_API_VERSION)
  count = visual_list_count (list);
  transcount = visual_list_count(translist);
#elif VISUAL_API_VERSION >= 4000 && VISUAL_API_VERSION < 5000
  count = visual_collection_size (VISUAL_COLLECTION (list));
  transcount = visual_collection_size (VISUAL_COLLECTION (translist));
#endif

  // Transformation plugins
  for(i = 0; i < transcount; i++) {
    VisPluginRef *ref = visual_list_get (translist, i);
    VisPluginData *visplugin = NULL;
    GType type;
    gchar *name;
    GTypeInfo info = {
